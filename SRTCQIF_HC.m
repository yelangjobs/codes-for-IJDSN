...
function [target_est,target_srcov,time_elapse] = SRTCQIF_HC(sensor_pos,measurement,init_est,init_est_cov)

global nx nz L consensus_matrix sensor_node;
...
for L_ = 1:L
    ...
    for k = 2:num_steps
        for i = 1:Ns
            [state_pred,srcov_pred] = SRTCKF_predict(target_est(:,k-1,i,L_),target_srcov(:,:,k-1,i,L_));
            state_info_pred(:,i) = (srcov_pred*srcov_pred')\state_pred;
            state_info_srcov_pred(:,:,i) = eye(nx)/srcov_pred';
            
            if ismember(i,sensor_node)
                [measure_info,measure_info_srcov] = SRTCIF_contribute(state_pred,srcov_pred,measurement(:,k,i),sensor_pos(:,i));
            else
        ...
        for iteration = 1:L_
            ...
            for i = 1:Ns
                consensus_binary(i) = consensus_matrix(i,:)*binary;
                
                weight_state_info_pred = state_info_pred*diag(consensus_matrix(i,:));
                consensus_pred(:,i) = sum(weight_state_info_pred,2);
                
                weight_srcov_pred_all = srcov_pred_all*kron(diag(sqrt(consensus_matrix(i,:))),eye(nx));
                consensus_srcov_pred(:,:,i) = Sroot(weight_srcov_pred_all);
                
                weight_meas_info = meas_info*diag(consensus_matrix(i,:));
                consensus_meas(:,i) = sum(weight_meas_info,2);
                
                if iteration == 1
                    Si_uk = meas_info_srcov_all*kron(diag(sqrt(consensus_matrix(i,:))),eye(nz));
                    consensus_srcov_meas(:,:,i) = Sroot(Si_uk);
                else
                    Si_uk = meas_info_srcov_all*kron(diag(sqrt(consensus_matrix(i,:))),eye(nx));
                    consensus_srcov_meas(:,:,i) = Sroot(Si_uk);
                end
            end
